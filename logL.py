...
for night in nights:
	for v,vmr in enumerate(vmrs):
		...
		# Read in data
		spec = np.load(data_path+night+'_spectra_sysrem_nodrift_10iters.npy')[iters[night]-1] - 1.
		wave = np.load(data_path+night+'_wavelength_blaze.npy')
		...
		# Get dayside model
		hdu = fits.open(model_path+'wasp33b_gui-3014.0K-inv-FastChem_FeI_logvmr%.1f.fits' % (vmr))
		model = hdu[0].data
		...
		# Perform cross-correlation for each order and sum together
